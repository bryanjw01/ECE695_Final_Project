...
from variables_ecog import TRAIN_DATA_PATH, VALIDATION_DATA_PATH, TESTING_DATA_PATH, MOCHA_PATH, COLUMN_WIDTH, PHONEME_PATH
import pdb

class DataPreprocessing(nn.Module):
    def __init__(self):
        super(DataPreprocessing, self).__init__()
        self.conv = nn.Conv1d(429, 64, 5, stride=5)
        self.linear = nn.Linear(252, 64)

    def forward(self, source):
        source = self.linear(self.conv(source.permute(0, 2, 1))).permute(0, 2, 1)
        return source#[:,None,:,:]
...
class Data_Formatting:
    ...
    def data_format(self, DATA_PATH):
        ...
        model = DataPreprocessing().to(DEVICE)
        for path in DATA_PATH:
            it = tf.compat.v1.python_io.tf_record_iterator(path)
            for i in it:
        ...
        x = list(x)
        x = np.array(model(torch.tensor(x).to(DEVICE)).cpu().detach())
        x = list(x)
