...
import math
from variables import COLUMN_WIDTH, TRG_PAD_IDX, SRC_PAD_IDX, device
import pdb
...
class LogRegression(nn.Module):
    def __init__(self, d_model):
        ...
        self.linear_1 = nn.Linear(d_model, 84)
        self.linear_2 = nn.Linear(84, 42)
        self.dropout = nn.Dropout(0.1)

    def forward(self, x):
        return self.linear_2(self.dropout(torch.relu(self.linear_1(x)[0])))
...
class Conv1d(nn.Module):
    def __init__(self, n_channels, stride, d_model):
        super(Conv1d, self).__init__()
        self.conv = nn.Conv1d(n_channels, d_model, stride, stride=stride)
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.feedForward = FeedForward(d_model, d_ff, P_drop, COLUMN_WIDTH)
        ...
        self.Conv = Conv1d(COLUMN_WIDTH, 12, d_model)
    ...
    def forward(self, source, mask):
        ...
        source = self.Conv(source.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        #  FeedFoward followed by Positional Encoding
        # output = self.feedForward(source) * math.sqrt(self.d_model)
        output = self.PositionalEncodingEcog(source)
...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_pad_idx, trg_pad_idx, d_model):
        '''
        ...
        self.regression = LogRegression(d_model)
    ...
    def forward(self, source, target):   
        ...
        reg_output = self.regression(encoder_output)
    
        decoder_output, _ = self.decoder(target, encoder_output, target_mask, source_mask)
        return decoder_output, reg_output